...
public class CustomSeekView extends FrameLayout implements Player.Listener, TimeBar.OnScrubListener {
    ...
    public void setMarkers(long opening, long ending, long duration) {
        if (duration <= 0) return;
        int count = 0;
        long[] times = new long[2];
        boolean[] played = new boolean[2];
        if (opening > 0) { times[count] = opening; count++; }
        if (ending > 0 && duration > ending) { times[count] = duration - ending; count++; }
        timeBar.setAdGroupTimesMs(times, played, count);
    }
