...
public class Action implements Process {
    ...
    private void onCast(Map<String, String> params) {
        ...
        // 【修复·P1 零点击崩溃】原来直接把可能为 null 的 device/history 投递出去，
        // 接收侧 ReceiveDialog.initView() 会立刻 NPE（局域网内任何设备发一个缺参数的
        // /action?do=cast 就能打崩电视端）。这里改为"数据不完整则不投递"。
        if (device == null || history == null) {
            Log.w("NanoAction", "cast ignored: incomplete params (device=" + (device != null) + ", history=" + (history != null) + ")");
            return;
        }
