...
public class FileUtil {
    ...
    public static void zipDecompress(File target, File path) {
        try (ZipFile zip = new ZipFile(target)) {
            // 【修复·P0 Zip Slip】逐条目做目录规范化校验：条目名里的 ../ 或绝对路径会落到
            // 目标目录之外（可覆盖应用私有目录下的可执行文件），这里直接跳过这类条目。
            File base = path.getCanonicalFile();
            Enumeration<?> entries = zip.entries();
            while (entries.hasMoreElements()) {
                ZipEntry entry = (ZipEntry) entries.nextElement();
                File out = new File(base, entry.getName()).getCanonicalFile();
                if (!out.getPath().equals(base.getPath()) && !out.getPath().startsWith(base.getPath() + File.separator)) continue;
