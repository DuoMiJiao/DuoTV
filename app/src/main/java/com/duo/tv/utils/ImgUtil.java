...
public class ImgUtil {
    ...
    public static void logo(ImageView view) {
        try {
            Glide.with(view).load(UrlUtil.convert(VodConfig.get().getConfig().getLogo())).circleCrop().override(ResUtil.dp2px(96), ResUtil.dp2px(96)).error(R.drawable.ic_logo).into(view);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    private static RequestListener<Drawable> getListener(String text, String url, ImageView view, boolean vod) {
        return new RequestListener<>() {
            ...
            public boolean onLoadFailed(@Nullable GlideException e, Object model, @NonNull Target<Drawable> target, boolean isFirstResource) {
                ...
                // 失败集合有界，网络恢复后会重新尝试
                if (failed.size() > 300) failed.clear();
                failed.add(url);
                return true;
            }
