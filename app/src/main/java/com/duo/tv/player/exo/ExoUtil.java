...
public class ExoUtil {
    ...
    public static void setPlayerView(PlayerView view) {
        ...
        if (Setting.getSubtitlePosition() != 0) view.getSubtitleView().setBottomPaddingFraction(0.081f + Setting.getSubtitlePosition());
    ...
    public static ExoPlayer buildPlayer(int decode, Player.Listener listener) {
        ExoPlayer player = new ExoPlayer.Builder(App.get()).setLoadControl(buildLoadControl()).setTrackSelector(buildTrackSelector()).setRenderersFactory(buildRenderersFactory(getRenderMode(decode))).setMediaSourceFactory(buildMediaSourceFactory()).setVideoChangeFrameRateStrategy(C.VIDEO_CHANGE_FRAME_RATE_STRATEGY_OFF).build();
